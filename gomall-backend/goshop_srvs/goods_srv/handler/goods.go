...
import (
	...
	"gomall/goshop_srvs/goods_srv/global"
	"gomall/goshop_srvs/goods_srv/model"
	"gomall/goshop_srvs/goods_srv/proto"
...
func (s *GoodsServer) GoodsList(ctx context.Context, req *proto.GoodsFilterRequest) (*proto.GoodsListResponse, error) {
	...
	if req.TopCategory > 0 {
		...
		if category.Level == 1 {
			subQuery = fmt.Sprintf("select id from category where parent_id in (select id from category where parent_id=%d)", req.TopCategory)
		} else if category.Level == 2 {
			subQuery = fmt.Sprintf("select id from category where parent_id=%d", req.TopCategory)
		...
		// 生成terms查询
		q = q.Filter(elastic.NewTermQuery("category_id", categoryIds))
	}
